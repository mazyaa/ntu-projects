...
@section('breadcrumb')
    <x-admin.page-header title="Kategori Riksa Uji" subtitle="Kelola kategori riksa uji K3." />
@endsection
    ...
    <x-card>
        <div class="p-6">
            <div class="flex items-center justify-between mb-6">
                ...
                <a href="{{ panel_route('riksa-uji-categories.create') }}" class="px-4 py-2 text-sm font-medium text-white bg-primary rounded-lg hover:bg-primary/90">
                    + Tambah Kategori
                </a>
            </div>
                ...
                <table class="w-full text-sm">
                    <thead>
                        <tr class="border-b border-gray-100">
                            ...
                            <th class="text-left py-3 px-4 font-semibold text-secondary">Tipe</th>
                    ...
                    <tbody>
                        @forelse ($categories as $category)
                            <tr class="border-b border-gray-50 hover:bg-gray-50">
                                <td class="py-3 px-4">
                                    <div class="font-medium text-secondary">{{ $category->name }}</div>
                                    @if($category->name_en)
                                        <div class="text-xs text-gray-400">{{ $category->name_en }}</div>
                                    @endif
                                </td>
                                <td class="py-3 px-4 text-gray-500 font-mono text-xs">{{ $category->slug }}</td>
                                <td class="py-3 px-4 text-gray-500">{{ $category->types->count() }} tipe</td>
                                <td class="py-3 px-4">
                                ...
                                <td class="py-3 px-4 text-right">
                                    <div class="flex items-center justify-end gap-2">
                                        <a href="{{ panel_route('riksa-uji-categories.edit', $category) }}" class="p-2 text-gray-400 hover:text-primary rounded-lg hover:bg-primary/10">
                                            <i data-lucide="pencil" class="w-4 h-4"></i>
                                        </a>
                                        <form method="POST" action="{{ panel_route('riksa-uji-categories.destroy', $category) }}" onsubmit="return confirm('Yakin ingin menghapus?')">
                                            @csrf @method('DELETE')
