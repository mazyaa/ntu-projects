    ...
    <div class="max-w-7xl mx-auto px-6 lg:px-8">
        <div class="text-center mb-16" data-aos="fade-up">
            <div class="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-primary/10 text-primary text-sm font-semibold mb-6">
                <i data-lucide="flask-conical" class="w-4 h-4"></i> {{ __('ui.leadership.badge') }}
            </div>
            <h2 class="text-3xl md:text-4xl font-bold text-secondary mb-4">{{ __('ui.leadership.h2') }}</h2>
            <p class="text-gray-500 max-w-2xl mx-auto">{{ __('ui.leadership.subtitle') }}</p>
        </div>
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8 max-w-5xl mx-auto">
            @php
                $delays = [100, 200, 300];
            @endphp
            @foreach(company('personnel', 'company-research') as $index => $person)
            <a href="{{ lroute('research') }}" class="group block glass-card rounded-3xl overflow-hidden hover:shadow-2xl hover:shadow-gray-200/50 transition-all duration-500 hover:-translate-y-2" data-aos="fade-up" data-aos-delay="{{ $delays[$index] }}">
                ...
                <div class="p-6 text-center">
                    ...
                    <div class="inline-flex items-center gap-1.5 px-3 py-1.5 bg-gray-50 border border-gray-100 rounded-full text-xs font-semibold text-gray-500 mb-4">
                        ...
                        {{ __('ui.leadership.projects_count', ['count' => count($person['projects'])]) }}
                    </div>
                    <div class="inline-flex items-center gap-1 text-sm font-bold text-primary transition-all duration-300 group-hover:gap-2">
                        {{ __('ui.leadership.view_record') }} <i data-lucide="arrow-right" class="w-4 h-4 group-hover:translate-x-1 transition-transform"></i>
                    </div>
                </div>
            </a>
            @endforeach
